            ...
            using (var requestData = PrepareRequestEnvelope(requestEnvelope))
            {
                using (var response = _httpClient.PostAsync(_requestUrl ?? Constants.ApiUrl, requestData).Result)
                {
                    ...
                    switch (responseEnvelope.StatusCode)
                    {
                        case 52: // Rate limit?
                                 //Log.Info(
                                 //$"We are sending requests too fast, sleeping for {Common.Configuration.RateLimitTimeout} milliseconds.");
                            System.Threading.Tasks.Task.Delay(Configuration.RateLimitTimeout).Wait();
                            //Thread.Sleep();
